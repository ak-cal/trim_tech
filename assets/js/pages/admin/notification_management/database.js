...
async function displayAppointments(data) {
    ...
    function populateDropdown(filteredData) {
        ...
        filteredData.forEach(appointment => {
            const customerName = appointment.Customers?.Users?.name || "Unknown Customer";
    ...
    data.forEach(notifs => {
        let row = `
            <tr>
                ...
                <td><button class="btn btn-delete btn-red" onclick="deleteNotification('${notifs.notification_id}')">Delete</button></td> 
            </tr>
            `;
